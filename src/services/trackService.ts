...
import { getArtistGroupOrDuo } from '../utils/artistUtil';
...
async function searchArtistGroupOrDuo(
  ...
  for (const groupName of groupsOrDuosNames) {
    ...
    const groupOrDuoIdresult =
      await SpotifyApiService.getInstance().client.searchArtists(groupName);
    ...
    const groupOrDuoAlbumsResult =
      await SpotifyApiService.getInstance().client.getArtistAlbums(
        groupOrDuoId
      );
    ...
    for (const album of groupOrDuoAlbums) {
      const albumTracksResult =
        await SpotifyApiService.getInstance().client.getAlbumTracks(album.id);
